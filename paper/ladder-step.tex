...
\begin{algorithm}[h]
  \begin{algorithmic}[1]  
    ...
    \begin{multicols}{2}
      \State $t\leftarrow P$
      \For{$i=\lfloor \log_2 n \rfloor-1\ \mathbf{downto}\ 1$}
  ...
  \end{algorithmic}
\end{algorithm}
That is from $[n]P$ before step 4, we first compute $[2n]P$
...
points, the Ladderstep starts from points $[n]P $ and
$[n+1]P $, to get either $ [2n]P $ and
$[2n+1]P$, or $[2n+1]P$ and $[2n+2]P$ in constant
time, the difference between the two points remaining $\pm P$.  This
is equivalent to computing either $[2n]P$ or $[2n+1]P$
from $[n]P$ and is sufficient to compute any $[n]P$ where
