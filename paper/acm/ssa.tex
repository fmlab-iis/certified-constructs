...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \Match{$e$}
      ...
      \Case{$e' + e''$}
        \Return \Call{SSAExpra}{$\theta$, $e'$} $+$
                \Call{SSAExpra}{$\theta$, $e''$}
      \EndCase
      \Case{$e' - e''$}
        \Return \Call{SSAExpra}{$\theta$, $e'$} $-$
                \Call{SSAExpra}{$\theta$, $e''$}
      \EndCase
      \Case{$e' \times e''$}
        \Return \Call{SSAExpra}{$\theta$, $e'$} $\times$
                \Call{SSAExpra}{$\theta$, $e''$}
      \EndCase
    \EndMatch
    \EndFunction
    ...
    \Match{$s$}
      \Case{$v \leftarrow a$}
        ...
        \State{\Return $\langle \theta'$,
                $v^{\theta'(v)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a$}$\rangle$}
      \EndCase
      \Case{$v \leftarrow a_1 + a_2$}
        \State{$\theta' \leftarrow \theta\mymapsto{v}{\theta(v) + 1}$}
        \State{\Return $\langle \theta'$,
                $v^{\theta'(v)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a_1$} $+$ \Call{SSAAtom}{$\theta$, $a_2$}$\rangle$}
      \EndCase
      \Case{$c\ v \leftarrow a_1 + a_2$}
        \State{$\theta' \leftarrow \theta\mymapsto{c}{\theta(c) + 1}$}
        \State{$\theta'' \leftarrow \theta'\mymapsto{v}{\theta(v) + 1}$}
        \State{\Return $\langle \theta''$,
                $c^{\theta'(c)} \ v^{\theta''(v)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a_1$} $+$ \Call{SSAAtom}{$\theta$, $a_2$}$\rangle$}
      \EndCase
      \Case{$v \leftarrow a_1 + a_2 + y$}
        \State{$\theta' \leftarrow \theta\mymapsto{v}{\theta(v) + 1}$}
        \State{\Return $\langle \theta'$,
                $v^{\theta'(v)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a_1$} $+$ \Call{SSAAtom}{$\theta$, $a_2$} $+$ $y^{\theta'(y)}$$\rangle$}
      \EndCase
      \Case{$c\ v \leftarrow a_1 + a_2 + y$}
        \State{$\theta' \leftarrow \theta\mymapsto{c}{\theta(c) + 1}$}
        \State{$\theta'' \leftarrow \theta'\mymapsto{v}{\theta(v) + 1}$}
        \State{\Return $\langle \theta''$,
                $c^{\theta'(c)} \ v^{\theta''(v)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a_1$} $+$ \Call{SSAAtom}{$\theta$, $a_2$} $+$ $y^{\theta(y)}$$\rangle$}
      \EndCase
      \Case{$v \leftarrow a_1 - a_2$}
        \State{$\theta' \leftarrow \theta\mymapsto{v}{\theta(v) + 1}$}
        \State{\Return $\langle \theta'$,
      ...
      \Case{$v \leftarrow a_1 \times a_2$}
        \State{$\theta' \leftarrow \theta\mymapsto{v}{\theta(v) + 1}$}
        \State{\Return $\langle \theta'$,
                $v^{\theta'(v)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a_1$} $\times$ \Call{SSAAtom}{$\theta$, $a_2$}$\rangle$}
      \EndCase
      \Case{$h\ l \leftarrow a_1 \times a_2$}
        \State{$\theta' \leftarrow \theta\mymapsto{h}{\theta(h) + 1}$}
        \State{$\theta'' \leftarrow \theta'\mymapsto{l}{\theta(l) + 1}$}
        \State{\Return $\langle \theta''$,
                $h^{\theta'(h)} \ l^{\theta''(l)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a_1$} $\times$ \Call{SSAAtom}{$\theta$, $a_2$}$\rangle$}
      \EndCase
      \Case{$v \leftarrow a << n$}
        \State{$\theta' \leftarrow \theta\mymapsto{v}{\theta(v) + 1}$}
        \State{\Return $\langle \theta'$,
                $v^{\theta'(v)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a$} $<<$ $n$$\rangle$}
      \EndCase
      \Case{$h\ l \leftarrow a@n$}
        \State{$\theta' \leftarrow \theta\mymapsto{h}{\theta(h) + 1}$}
        \State{$\theta'' \leftarrow \theta'\mymapsto{l}{\theta(l) + 1}$}
        \State{\Return $\langle \theta''$,
                $h^{\theta'(h)} \ l^{\theta''(l)} \leftarrow$ \Call{SSAAtom}{$\theta$, $a$} $@$ $n$$\rangle$}
      \EndCase
      \Case{$h\ l \leftarrow (a_1.a_2)@n$}
        \State{$\theta' \leftarrow \theta\mymapsto{h}{\theta(h) + 1}$}
        \State{$\theta'' \leftarrow \theta'\mymapsto{l}{\theta(l) + 1}$}
        \State{\Return $\langle \theta''$,
                $h^{\theta'(h)} \ l^{\theta''(l)} \leftarrow$ (\Call{SSAAtom}{$\theta$, $a_1$} $.$ \Call{SSAAtom}{$\theta$, $a_2$}) $@$ $n$$\rangle$}
      \EndCase
    \EndMatch
    \EndFunction
  \end{algorithmic}
