...
We call a statement $s \in \bvStmt$ \emph{safe} at a state $\bvst \in \bvSt$, denoted by $\safe(s)$, if for every state $\bvst \in \bvSt$, \textsc{StmtSafe}($s$, $\bvst$) evaluates to $\btt$.
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \Match{$s$}
      ...
      \Case{$v \leftarrow a_1 - a_2$}
        \State{\Return $\bvvalueof{a_2}(\bvst) \ll_\BV \bvvalueof{a_1}(\bvst)$}
      \EndCase
...
Assume $\safe(\dslcode{bSub})$ holds.
