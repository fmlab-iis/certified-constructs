...
\bvst & \goesto{v_h\ v_l \leftarrow (a_1 . a_2) @ n} & \bvst[v_h \leftarrow \hi_\bvop((\bvvalueof{a_1}(\bvst) ._\bvop \bvvalueof{a_2}(\bvst)) \ll_\bvop \unsigned{n})][v_l \leftarrow (\lo_\bvop((\bvvalueof{a_1}(\bvst) ._\bvop \bvvalueof{a_2}(\bvst)) \ll_\bvop \unsigned{n})) \gg_\bvop \unsigned{n}] \\
\end{array}
\]
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \Match{$s$}
      ...
      \Case{$v \leftarrow a_1 - a_2$}
        \State{\Return $\bvvalueof{a_2}(\bvst) \leq_\BV \bvvalueof{a_1}(\bvst)$}
      \EndCase
      ...
      \Case{$v \leftarrow a \ll n$}
        \State{\Return $\bvvalueof{a}(\bvst) <_\BV (\bv^\wordsize(1) \ll_\BV (\wordsize - n))$}
      \EndCase
      \Case{$v_h\ v_l \leftarrow a @ n$} \Return $\btt$ \EndCase
      \Case{$v_h\ v_l \leftarrow (a_1.a_2)@n$}
        \State{\Return $\bvvalueof{a_1}(\bvst) <_\BV (\bv^\wordsize(1) \ll_\BV (\wordsize - n))$}
      \EndCase
    \EndMatch
    \EndFunction
  \end{algorithmic}
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \Match{$p$}
      ...
      \Case{$s; pp$} \Return \Call{StmtSafe}{s} $\wedge$ \Call{ProgSafe}{pp} \EndCase
    \EndMatch
    \EndFunction
  \end{algorithmic}
