Algebraic specifications in \bvdsl are transformed to polynomial polynomial equation entailment problems via an intermediate language \zdsl.
...
For algebraic expressions, since $\bvExpa$ is a sub-class of $\zExpr$, we can easily define a function \textsc{BV2ZExpr} that converts an algebraic expression $e_a \in \bvExpa$ to $\zExpr$ such that for every $\bvst \in \bvSt$ and $\zst \in \zSt$ with $|\bvst(v)| = \zst(v)$ for all variables v (written as $\bvst \simeq \zst$), $\zvalueof{e_a}(\bvst) = \zvalueof{\textsc{BV2ZExpr}(e_a)}(\zst)$.
...
Functions \textsc{BV2ZAtom} (Algorithm~\ref{algorithm:bv2z-atom}), \textsc{BV2ZStmt} (Algorithm~\ref{algorithm:bv2z-stmt}), and \textsc{BV2ZProg} (Algorithm~\ref{algorithm:bv2z-prog}) are defined to respectively transform an atom, a statement, and a program in \bvdsl to an expression, a statement, and a program in \zdsl.
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \Match{$s$}
      ...
      \EndCase
      \Case{$v_h\ v_l \leftarrow (a_1.a_2)@n$}
