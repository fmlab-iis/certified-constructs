...
Definition x25519_x86_64_square_post : bexp :=
  let  a0 := 1000 in
  let  a1 := 1001 in
  let  a2 := 1002 in
  let  a3 := 1003 in
...
bvAdc r12 (bvVar rdx) (bvVar r12) carry;
...
(* NOTE: ignore carry because it is zero *)
bvAdc r14 (bvVar rdx) (bvVar r14) carry;
bvAssign carry (bvConst (fromPosZ 0%Z));
(* mov    0x8(%rsi),%rax                           #! EA = L0x603068 *)
bvAssign rax (bvVar L0x603068);
(* shl    %rax *)
...
(* NOTE: ignore carry because it is zero *)
bvAdc rbx (bvVar rdx) (bvVar rbx) carry;
bvAssign carry (bvConst (fromPosZ 0%Z));
...
(* NOTE: ignore carry because it is zero *)
bvAdc r8 (bvVar rdx) (bvVar r8) carry;
bvAssign carry (bvConst (fromPosZ 0%Z));
...
(* NOTE: ignore carry because it is zero *)
bvAdc r10 (bvVar rdx) (bvVar r10) carry;
bvAssign carry (bvConst (fromPosZ 0%Z));
...
Definition x25519_x86_64_square_spec :=
  {| spre := check_pre;
     sprog := program;
     spost := x25519_x86_64_square_post |}.

Lemma valid_x25519_x86_64_square :
  valid_bvspec (check_inputs, x25519_x86_64_square_spec).
Proof.
